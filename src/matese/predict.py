...
def main(args):
    ...
    metric = MaTESe.load_metric(args.metric, batch_size=args.batch_size, device=device)
    ...
    if (sources and candidates and metric.reference_less) or (
        references and candidates and not metric.reference_less
    ):

        predictions = metric.evaluate(candidates, sources=sources, references=references)
